...
| 对比  | kafka | RocketMQ | ActiveMQ | redis            |
|-----|-------|-----|------------------|------------------|
| 开发语言 | scala（Linkedin开发的） | Java | Java | C语言              |
| 吞吐量 | 百万级(磁盘顺序读写)，topic数量多时性能下降非常严重，不适合复杂业务系统 | 十万级，topic数量多时性能下降不严重，适合复杂业务系统 | 万级(磁盘随机读写) | 十万级              |
| 游标位置 | 客户端自己管理，可重复读取| 客户端自己管理，可重复读取 | ActiveMQ来管理，无法读取历史数据 | 不支持历史数据重复消费      |
| 时效性 |    ms级 | ms级 |  ms级| ms级 |
| 工作模型 | 只有topic，但是每个topic可以有很多partition分区 | topic |  queue、topic | 订阅一个或多个频道(topic) |
