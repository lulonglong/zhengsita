...
```
 ![alt](https://pics2.baidu.com/feed/caef76094b36acafc28191712593721800e99ca9.png@f_auto?token=8ee5889362ea2bb3144ddc567e513b41)
...
※由上可知，分库分表并不是一起出现的，要根据实际情况进行应用。在项目设计初期，根据项目的预估容量来判断瓶颈在哪里，瓶颈在表就分表，瓶颈在库就分库，两个都有瓶颈就即分库又分表，如果没有瓶颈就不要瞎鸡巴分
```
 ![alt](https://pic1.zhimg.com/80/v2-2ba664c804fcad455cb17b2502825ec4_720w.webp?source=1940ef5c)
...
分库的时候除了要考虑平时的业务峰值读写QPS外，还要考虑到诸如双11大促期间可能达到的峰值，需要提前做好预估。预估总并发和连接数，了解单库的并发和连接数，得出分库的数量。一般连接数根据cpu的数量乘以10以内即可，上线之前在这附近设置几个档位做下压测看下实际效果。以一条的数据库实例为例，cpu是4核，实例那边最大是1000多。客户端连接数最大是maxActive=100，这个其实需要根据实力最好性能对应的连接数除以机器数量再稍微加一点即可，100太大了。
...
2. 中间层代理中间件
		这类分库分表中间件的核心原理是在应用和数据库的连接之间搭起一个代理层，上层应用以标准的MySQL协议来连接代理层，然后代理层负责转发请求到底层的MySQL物理实例，这种方式对应用只有一个要求，就是只要用MySQL协议来通信即可，所以用MySQL Workbench这种纯的客户端都可以直接连接你的分布式数据库，自然也天然支持所有的编程语言。比较有代表性的产品有开创性质的Amoeba、Sharding-Proxy、阿里开源的Cobar、社区发展比较好的Mycat 等
```
...
等数据同步完毕后
暂停主库写入，把从库断开同步，作为写库，切换程序的分库分表策略（可以做成开关）
...
```
 ![alt](https://pic1.zhimg.com/80/v2-4f50890c237a2f16cc044c9ca19458c4_720w.webp)
...
第四步：（同步双写）应用去掉双写，部署；
```
 ![alt](https://pic3.zhimg.com/80/v2-7a4dea220c85d2ecabef6175ba8f05ae_720w.webp)

```
③range加hash的方式，把每次扩容的机器作为一个组
最初的分库规模可以认为是Group1，到瓶颈之后做扩容，这个时候把新扩容的实例作为Group2
推荐使用这种方法，简单易搞还不需要数据迁移
```
 ![alt](https://pic4.zhimg.com/v2-964dd69872982338f75c0561f06df66f_b.jpg)
 ![alt](https://pic3.zhimg.com/v2-651a441aa360f11339ac54405be0ac4a_b.jpg)
...
问题：对数据的写入逻辑有侵入
```
 ![alt](https://pic1.zhimg.com/80/v2-c6216d1afd8b954f218a1a1d40193078_720w.webp)
...
取模要用最后几位基因来取
```
 ![alt](https://pic2.zhimg.com/80/v2-9106978546e908dece01496fb855cf45_720w.webp)
```
③数据副本
图中的NoSQL也可以用关系数据库，因为后台的查询并发会小很多。一般情况用ES，后台查询的模糊查询需求可能比较多
```
 ![alt](https://pic3.zhimg.com/80/v2-ca50b08d27e1e0c902ec124d41b59c4e_720w.webp)
...
```
 ![alt](https://pic4.zhimg.com/80/v2-a5d1274daf67b958e46459b53978391b_720w.webp)
...
```
 ![alt](https://pic2.zhimg.com/80/v2-dee3b974fc615bf5c1903d2dad4ea869_720w.webp)

## Sharding-JDBC解读
```
官方文档：https://shardingsphere.apache.org/document/current/cn/overview/
```
 ![alt](https://pics2.baidu.com/feed/3b87e950352ac65cb0a73717a6b84d1992138a83.jpeg@f_auto?token=ff6036f2483b5d5f8a1b4370802d5c30)

###### 1. 执行流程
 ![alt](https://pics2.baidu.com/feed/562c11dfa9ec8a13fa8e5a89ad496e87a1ecc092.jpeg@f_auto?token=7105e51bcb198775c180aee72e997bcc)
...
sharding-jdbc提供了常用的表达式配置、hash取模、range等分片算法，并支持自定义分片算法，如下是一个表达式方式的分片算法，用表达式实现了取模分片
```
```xml
...
```
 ![alt](https://shardingsphere.apache.org/document/current/img/readwrite-splitting/challenges.png)
```xml
<!--从库负载均衡算法-->
...
另一种情况是当请求在事务中先写入数据，然后查询数据，这个时候如果走从库是不可能查到的，因为事务并未提交。这种情况是中间件可以帮我们解决的。
...
```
 ![alt](https://pic1.zhimg.com/v2-44ff9ce47771fc8222f0df77ab3f0b30_b.jpg)
...
其实就是二阶段提交。
```
 ![alt](https://pics6.baidu.com/feed/aec379310a55b31986a0f72ca368ec2ecefc1753.jpeg@f_auto?token=e1ef369959b2d589b4ec4daee38268a2)
...
```
 ![alt](https://shardingsphere.apache.org/document/current/img/encrypt/4.png)
