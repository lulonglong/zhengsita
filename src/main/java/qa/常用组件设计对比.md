...
**Redis6的多线程模型**
```
主线程也参与IO线程的任务，只是在IO方面多了几个线程来分担IO的操作，这个与主从Reactor模型不一致
```
...
| 组件  | 实现方法             | 优缺点                                                       |
| ----- | -------------------- | ------------------------------------------------------------ |
| Kafka | 专业的分布式消息系统 | 高吞吐量，高可用，支持partition                              |
