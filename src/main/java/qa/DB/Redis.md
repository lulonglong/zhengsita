...
* 过期策略：Memcache 过期后，不删除缓存，会导致下次取数据的问题，Redis 有专门线程，清除缓存数据；
...
Redis 的单线程模型，指的是执行 Redis 命令的核心模块是单线程的，而不是整个 Redis 实例就一个线程。
...
怎么理解
在 MySQL 中，事务是指一组操作中，要么全部执行，要么全部不执行。而在 Redis 也存在事务的概念，Redis 的事务可以保证一次执行多个命令，每个事务是一个单独的隔离操作，事务中的所有命令都会序列化、按顺序地执行。服务端在执行事务命令的过程中，不会被其他客户端发送来的命令请求打断。
...
使用最多的淘汰策略
volatile-lru 从已设置过期时间的key中找出最近最少使用的淘汰
...
### 什么是缓存击穿
缓存击穿是指某一个热点key，访问量超级大，过期的一瞬间导致大量流量下沉到数据库，给数据库造成压力。解决办法是使用逻辑过期和物理过期，但是要封装缓存对象（只针对特定操作封装）。还有一种方式是去数据库拉取的时候加jvm锁，这样数据库的流量就只有机器数量的并发量了，其他被阻塞拉数据库的请求在获取到锁之后再一次先到redis拉。逻辑流程是拉redis==null-》 加锁-》再拉redis-》没有再拉数据库
...
区别
* codis 是豌豆荚开发的，是采用中心化集群的方式，由代理层proxy来进行转发，它内部维护了请求转发的具体规则，Codis把整个集群划分为1024个槽位，在处理读写请求时，采用crc32 Hash算法计算key的Hash值，然后再根据Hash值对1024个槽位取模，最终找到具体的Redis节点。
...
每次从数据库取 key 的时候检查 key 是否过期，如果过期则删除，并返回 null，如果 key 没有过期，则直接返回数据。（只有主库会删除，从库不删除）
