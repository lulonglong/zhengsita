...
为什么选用B+树，B+树的叶子结点冗余了所有索引key，每个节点间都有首尾引用，方便范围查找
```
...
Extra(重要)：此列是一些额外信息。常见的重要值如下
		...
		2）Using index condition：与Using index的区别是where条件同样都命中索引，差别是返回的列不完全被索引覆盖，需要回表拿其他列的信息
		3）Using where：需要做过滤（这过滤的含义是除直接命中唯一索引外的where条件），不管是覆盖索引还是非覆盖索引，只要需要过滤的就有Using where
...
12. update的返回值代表啥
```
1.当数据库的url中没有“useAffectedRows=true”参数时，返回匹配行数；
