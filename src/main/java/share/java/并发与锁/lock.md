...
​		`LockSupport`是一个线程阻塞工具类，所有的方法都是静态方法，可以让线程在任意位置阻塞，当然阻塞之后肯定得有唤醒的方法。归根结底，LockSupport是调用的Unsafe的native代码。
...
AQS全称 AbstractQueuedSynchronizer，即抽象的队列同步器，是一种用来构建锁和同步器的框架。简单理解就是把争用锁的线程放到队列，管理线程对锁的获取。AQS是并发锁实现的基础，用CAS代替重量级锁。
...
​		如果被请求的共享资源空闲，则将当前请求资源的线程设置为有效的工作线程，并且将共享资源设置为锁定状态。如果被请求的共享资源被占用，那么就需要一套线程阻塞等待以及被唤醒时锁分配的机制，这个机制AQS是用CLH队列锁实现的，即将暂时获取不到锁的线程加入到队列中。这里要理解下背景，为什么用CLH队列锁，而不是直接在锁定资源上CAS等待，因为对同一个资源进行高并发的CAS会引起CPU总线风暴，CAS的底层实现是用汇编指令*cmpxchg*，在多核cpu中会为cmpxchg加上lock前缀，熟悉volatile的就知道lock前缀的作用可以使缓存行在其他核的状态变为失效，变为失效这个动作要通过CPU总线发信号，如果是8核CPU，一个线程CAS成功，那就要通知其他7个CPU缓存失效，频繁发生会引起大量的总线占用。用CLH可以解决这个问题，后边的线程只需要CAS前一个线程的状态即可，相当于CAS的热点不存在了。ZK中的分布式锁也是同样的实现思路。如图所示：

![img](../assets/9abebc6247074cc5571dac5259322aff.png)

**Sync queue：** 同步队列，是一个双向列表。包括head节点和tail节点。head节点主要用作后续的调度。

![img](../assets/ad0a72a0627c0daf5c31cf3d63cde7bc.png)

**Condition queue：** 非必须，单向列表。当程序中存在condition的时候才会存在此列表。此处最好先理解Condition的概念，我们在使用synchronized的时候，实际上锁对象是普通的java对象，也就是每个java对象都是内置锁对象。除此之外，每个锁对象背后还有一个锁对象监视器objectMonitor。用来管理线程阻塞和释放。那如果不用synchronized而是自定义一个并发锁，例如ReentrantLock。那谁来作为objectMonitor，答案就是Condition，查看一下ReentrantLock的用法就知道，如果释放锁或者唤醒等待线程，都要用Condition。

![img](../assets/ade8cc3617c8b5c3ac6b2da35e3fe6be.png)

#### **AQS设计思想**

- AQS使用一个int成员变量来表示同步状态，这个同步状态在实现的锁中可以作为不同的用途
- 使用Node实现FIFO队列，可以用于构建锁或者其他同步装置，Node封装了当前线程
- AQS资源共享方式：独占Exclusive（排它锁模式）和共享Share（共享锁模式）

```
AQS的所有子类中，要么实现并使用独占功能的api，要么使用共享锁的功能，而不会同时使用两套api，即便是最有名的子类ReentrantReadWriteLock也是通过两个内部类读锁和写锁分别实现了两套api来实现的
```
...
![img](../assets/568e72da9bcad9bcd41edf5bdb87e2f0.png)

 独占模式下的AQS是不响应中断的，指的是加入到同步队列中的线程，如果因为中断而被唤醒的话，不会立即返回并且抛出InterruptedException。而是再次去判断其前驱节点是否为head节点，决定是否争抢同步状态。如果其前驱节点不是head节点或者争抢同步状态失败，那么再次挂起。 

![Linux 中的等待队列机制](../assets/Linux中的等待队列机制.png)
...
![img](../assets/99551e3c9965ea44bade764f9124b131.png)
...
public final boolean release(int arg) {
    if (tryRelease(arg)) {
        ...
        if (h != null && h.waitStatus != 0)
          	//可以看下这个方法的代码，从队列尾部开始往前遍历唤醒，考虑下为什么
...
![img](../assets/68223fcc9e4aeb67bb01f0080fc0d0f0.png)
...
![img](../assets/ReentrantLock和Synchronized的对比.png)
...
读写锁是通过一个 int 类型的状态，来记录读锁状态和写锁状态

![ReentrantReadWriteLock读写锁详解-冯金伟博客园](../assets/20220923040135181.png)
...
![img](../assets/201c2a84d7d8fbdc.png)
...
![img](../assets/26273155-82e0eaefe6098fcf.png)

- t2 拿不到锁，进行排队

![img](../assets/26273155-7120772dbaf15d3d.png)

- t1 释放写锁

![img](../assets/26273155-c769e56e18ca6746.png)



此时t2 获得了锁，但是不是exclusive 的， state 现在为0

- 在t1 release 时候，unpark t2，t2 里从park状态激活，state =1_0

![img](../assets/26273155-61c6591e99679826.png)
...
再进入doReleaseShared，在此时释放t2. t2 节点从队列去除。 t3 线程被unpark，  t3 节点成为头结点。 当它进入doAcquireShared->setHeadAndPropagate 进入不了doReleaseShared， 因为t4 不是shared 的，所以t4 线程不能被激活。在此时state 为2_0
 ![img](../assets/26273155-1cd340eda3ef0954.png)
