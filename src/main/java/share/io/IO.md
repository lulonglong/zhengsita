...
![](https://yit-integration.oss-cn-hangzhou.aliyuncs.com/CRM/TECH/%E9%98%BB%E5%A1%9EIO1.webp)
...
![img](https://upload-images.jianshu.io/upload_images/19313705-0f316bbf4ca75281.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
![](https://yit-integration.oss-cn-hangzhou.aliyuncs.com/CRM/TECH/%E9%9D%9E%E9%98%BB%E5%A1%9EIO.webp)
...
![](https://yit-integration.oss-cn-hangzhou.aliyuncs.com/CRM/TECH/%E5%A4%9A%E8%B7%AF%E5%A4%8D%E7%94%A8%E5%9E%8BIO.webp)
...
![img](https://upload-images.jianshu.io/upload_images/19313705-8ee83caa6f0eca07.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
 ...
 **（1）select 调用需要传入 fd 数组，需要拷贝一份到内核，高并发场景下这样的拷贝消耗的资源是惊人的。（可优化为不复制）
 ...
 ![](https://upload-images.jianshu.io/upload_images/19313705-6a65b5ff9bf09bca.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
![epoll的三大步骤图](https://img.inotgo.com/imagesLocal/202204/12/202204121402390435_2.png)

###### **Epoll第一步epoll_create:**

当某个进程调用epoll_create方法时，内核会创建一个eventpoll对象（epfd文件描述符）

![在这里插入图片描述](https://img.inotgo.com/imagesLocal/202204/12/202204121402390435_8.png)
...
![在这里插入图片描述](https://img.inotgo.com/imagesLocal/202204/12/202204121402390435_3.png)

当socket收到数据后，中断程序会操作eventpoll的就绪队列rdlist，而不是直接操作读取数据的进程（如进程A）。当socket2、socket3收到数据后，中断程序让这两个socket进入rdlist。



![在这里插入图片描述](https://img.inotgo.com/imagesLocal/202204/12/202204121402390435_5.png)

**Epoll的第三步：epoll_wait:**
当程序执行到epoll_wait时，如果rdlist非空则返回，如果rdlist为空则阻塞进程。

![在这里插入图片描述](https://img.inotgo.com/imagesLocal/202204/12/202204121402390435_6.png)
...
![](https://yit-integration.oss-cn-hangzhou.aliyuncs.com/CRM/TECH/%E4%BF%A1%E5%8F%B7IO.webp)
...
​		信号驱动I/O是指进程预先告知内核，向内核注册一个信号处理函数，然后用户进程返回不阻塞，当内核数据就绪时会发送一个信号给进程，用户进程便在信号处理函数中调用I/O读取数据。从上图可以看出，实际上I/O内核拷贝到用户进程的过程还是阻塞的，信号驱动I/O并没有实现真正的异步，因为通知到进程之后，依然由进程来完成I/O操作。这和后面的异步I/O模型很容易混淆，需要理解I/O交互并结合五种I/O模型进行比较阅读。信号驱动I/O模型的特性总结如下表所示
 ...
 ![](https://upload-images.jianshu.io/upload_images/19313705-86e0f1b253ea7ab4.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
![](https://yit-integration.oss-cn-hangzhou.aliyuncs.com/CRM/TECH/%E5%BC%82%E6%AD%A5IO.webp)
...
![img](https://upload-images.jianshu.io/upload_images/19313705-07db7ab264dc1711.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)



从上图可以看出，阻塞程度：阻塞I/O>非阻塞I/O>多路复用I/O>信号驱动I/O>异步I/O，效率是由低到高的。最后，再看一下下表，从多维度总结了各I/O模型之间的差异，可以加深理解。

![img](https://upload-images.jianshu.io/upload_images/19313705-b52ab928bdb2d8c3.png?imageMogr2/auto-orient/strip|imageView2/2/w/1200/format/webp)
...
![img](https://pic4.zhimg.com/80/v2-82766de5d99af36aac845107baa62c17_720w.jpg)
...
在2011年7月28日，jdk1.7被正式发布。主要是引入了AIO的实现
