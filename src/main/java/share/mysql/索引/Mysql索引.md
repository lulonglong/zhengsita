...
B+树在B树的基础上做了改进
1.键值全部下沉到叶子结点做冗余
2.叶子结点之间使用指针首尾相连，注意：非叶子结点在同一层也做了首尾相连
...
## 索引分裂
```
从B-树的插入操作可以看出，当节点满员时，节点会一分为二，并且是按照50%分割，从参考文章的算法动画中看更加直观。这种方式的好处是，如果插入值比较随机的话，会减少索引分裂的概率，每个页面都会最大程度的被数据占用。但是考虑一种最常见的情况，我们几乎每个表都会有主键，那么在插入数据时都是主键递增插入，如果按照这种50%分裂的方式，就会出现所有的节点利用率都只有50%，造成大面积的空间浪费。MySQL利用数据页中记录插入方向和连续插入次数来优化这个问题，优化后如果按照主键插入，当节点满员时直接开辟新的节点放入新数据。具体参考mysql/存储结构中的页结构里Page Header的属性PAGE_DIRECTION、PAGE_N_DIRECTION
```

## 注意事项
```
>>Innodb存储引擎
不区分索引和数据，聚簇索引存储所有行记录
二级索引的非叶子节点存储的是[key，主键，页号]
二级索引的叶子节点存储的是[key，主键]
※根据主键再走一遍聚簇索引

>>myisam存储引擎
区分索引和数据文件，主键索引也是二级索引，没有一级索引一说
二级索引的非叶子节点存储的是[key，行号，页号]
二级索引的叶子节点存储的是[key，行号]
※直接根据行号去数据文件定位记录

myisam的数据行是顺序写入存储的，innodb要根据主键排序
```
...
[从MySQL Bug#67718浅谈B+树索引的分裂优化](https://www.cnblogs.com/mscm/p/13493129.html)
