...
## 底层数据结构
![alt 底层数据结构示意图](https://img-stage.yit.com/CMSRESQN/9576c6baeef47913325907b150a144a5_2296X1356.png)
...
```
![alt redisObject数据结构示意图](https://img-stage.yit.com/CMSRESQN/241883e0a37a2b5008699616247e124b_1528X712.png)
...
#### refcount
refcount属性，是对象的引用计数，显然计数0那么就是可以回收。
...
![alt SDS概览图](https://img-stage.yit.com/CMSRESQN/6ecf8a53cd6e6bdbb63bd3093f2a5cac_1066X148.png)

其中sdshdr是头部, buf是真实存储用户数据的地方。这个数据结构除了能存储二进制数据, 显然是用于设计作为字符串使用的, 所以在buf中, 用户数据后总跟着一个\0. 即图中 "数据" + "\0"是为所谓的buf。

#### sdshdr
![alt sdshdr概览图](https://img-stage.yit.com/CMSRESQN/2a15ee7e25918776a508dd2af2bfb4c3_1572X304.png)

![alt sds数据结构](https://img-stage.yit.com/CMSRESQN/8aae44e10fdbfd2ec7a29494bc086ce3_2166X1354.png)
...
![ZipLisT的存储结构](https://img-stage.yit.com/CMSRESQN/3726d69f6820d63d17d1ecaca1548f76_1556X234.png)
...
#### zlentry
先看源码
![Entry源码](https://img-stage.yit.com/CMSRESQN/78f6dd4b9ff2f3e4cacbe317e7be6a5b_2010X1064.png)
...
* redis中，在存储数据时，会先尝试将string转换成int存储，节省空间。此时entry结构：<prevlen> <encoding>

prevlen编码
* 当前一个元素长度小于254（255用于zlend）的时候，prevlen长度为1个字节，值即为前一个entry的长度
...
![quicklist的内存布局图](https://img-stage.yit.com/CMSRESQN/5661d1c7e8d103a552ffe1756b842566_2572X1260.png)

* quicklist.fill
  * 影响着每个链表结点中, ziplist的长度. 
...
#### 整数集合的升级
![intset内存图](https://img-stage.yit.com/CMSRESQN/354bbfed098d4504736edbda57a22942_1554X184.png)
...
![avator](https://img-stage.yit.com/CMSRESQN/5fca0945a4d266fa70575a94ec202023_2804X526.png)

如果我们增加如下两级索引，那么它搜索次数就变成了3次
![avator](https://img-stage.yit.com/CMSRESQN/e1b6f89a327e25a6c2cb1c214c5db3c8_1510X838.png)
...
内存布局如下图
![内存布局图](https://img-stage.yit.com/CMSRESQN/ecd26f18c43e91be930c34cea2c5b906_1588X1208.png)
...
#### 内存布局
![三种内存布局图](https://img-stage.yit.com/CMSRESQN/a81d6ade297eb26c6bf890cf8578089a_1516X1018.png)
...
#### 内存布局
![内存布局](https://img-stage.yit.com/CMSRESQN/99bceb7e6b0087ecf793ac640258600c_1500X868.png)
...
#### 内存布局
![内存布局](https://img-stage.yit.com/CMSRESQN/a8be0adb1c42972fc7830350d5b46188_1584X832.png)
...
#### 内存布局
![内存布局](https://img-stage.yit.com/CMSRESQN/9c0e4d9c33247641bfa4690dd03ac7e1_1530X950.png)
...
ziplist作为底层实现的内存分布
![avator](https://img-stage.yit.com/CMSRESQN/58486a0d15fa7fab32a5118ffa80b97e_1292X356.png)

skiplist和dict作为底层实现的内存分布
![avator](https://img-stage.yit.com/CMSRESQN/935fd4e0de8761c123e0f1738cb00dc4_2102X1378.png)
