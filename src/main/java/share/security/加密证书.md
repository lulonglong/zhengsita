...
## 数字签名
  ...
  * 签名
    * 先计算数据的摘要，再使用私钥对摘要进行加密生成签名，将 数据和签名 一并发送给接收方
  * 验签
    * 先使用相同的摘要算法计算接收数据的摘要，再使用预先得到的公钥解密签名，对比 解密的签名 和 计算的摘要 是否一致。若一致，则说明数据没有被篡改
...
### 存在的背景
  * 用来安全的发放公钥
  * 防止篡改和冒充，需要依靠数字签名来实现。但是数字签名的前提是，对方拥有公钥。但是在发放的公钥的过程中，也可能被冒充，冒充者把自己的公钥，发给了对方。加密证书就是为了解决这个问题而存在的。
...
### 概念
  * 数字证书也可以看做公钥的数字签名。CA用自己的私钥，对申请证书者提交的持有人信息、公钥等信息，加签生产了公钥证书。CA的公钥证书，可以进行验签通过。从而获取没有被篡改和为伪造的公钥。
...
* ![结构图](https://pic1.zhimg.com/v2-9b7cbd5eb1b56c6e6632f1ccb1c4a448_r.jpg)
